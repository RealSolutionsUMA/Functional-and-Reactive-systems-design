...
a one to one relation betweeen a function call and the result it produces. For
...
expression that its trivial to solve [[Include example]].
...
time it is not possible to ensure that without changes to the function itself,
the functionality will keep the same, as changes in the context of the function
regarding the variable \texttt{name} may change the behaviour of the function in
the future [[Introduce new example]].
...
the only thing its needed to do is to start changing the calls for results they
producce. [[Do and follow an example of RT]]
...
start substituting. For every substitution, it is checked that the result of it
is the one expected. At the moment that a function results to a non expected
result the cause of the error is found.
...
Scala REPL and GHCi for Haskell. An example of use is shown [[Introduce
example]].
